...
void PLOTTING::plotPlacement(std::string imageName, PlaceData* db, MyPlacer* placer)
{
    using std::cout;
    using std::endl;
    ...
    const unsigned char Yellow[3] = {255, 255,   0};  // 黄色用于 filler
    ...
    // 5) 绘制 filler 单元（如果提供了 placer）
    // 从 NodesAndFillers 中绘制 filler，确保使用更新后的位置
    if (placer && !placer->NodesAndFillers.empty()) {
        //
        int fillerDebugCount = 0;
        const int MAX_DEBUG_PRINT = 5;
        //
        for (Module* curFiller : placer->NodesAndFillers) {
            if (!curFiller || !curFiller->isFiller) continue;

            // 抽样打印前 5 个 filler 的信息
            if (fillerDebugCount < MAX_DEBUG_PRINT) {
                std::cout << "[Filler #" << fillerDebugCount << "] center=(" 
                          << curFiller->center.x << ", " << curFiller->center.y 
                          << "), width=" << curFiller->width 
                          << ", height=" << curFiller->height << std::endl;
                fillerDebugCount++;
            }
            //
            double llx = (double)curFiller->center.x - (double)curFiller->width  * 0.5;
            double lly = (double)curFiller->center.y - (double)curFiller->height * 0.5;
            double urx = (double)curFiller->center.x + (double)curFiller->width  * 0.5;
            double ury = (double)curFiller->center.y + (double)curFiller->height * 0.5;
            ...
            // filler 单元用黄色显示
            img.draw_rectangle(x_left, y_top, x_right, y_bottom, Yellow, opacity);
        }
    }
