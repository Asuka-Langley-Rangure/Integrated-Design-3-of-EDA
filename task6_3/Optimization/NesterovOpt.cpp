...
void NesterovOpt::init()
{
    ...
    // ===== 保存初始布局（iter=0）=====
    PLOTTING::plotPlacement("iteration/place_iter_0", placer->db);
...
void NesterovOpt::NAG_Step()
{
    ...
    // 每 10 次迭代保存一张图，避免频繁 I/O
    if (iter_count % 10 == 0)
    {
        // Ensure DB uses the updated reference solution before plotting/HPWL logging.
        pushPositionToPlacer(cur_iter.reference_solution);

        std::string iterImg = std::string("iteration/place_iter_") + std::to_string(iter_count);
        PLOTTING::plotPlacement(iterImg, placer->db);
    }
