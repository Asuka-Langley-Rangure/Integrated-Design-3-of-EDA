...
void MyPlacer::initialPlacement()
{
    ...
    for (Module* mod : db->Nodes) {
        ...
        modToIndex[mod] = (int)movableMods.size();
        movableMods.push_back(mod);
    }
    int N = (int)movableMods.size();
    if (N == 0) return;
    ...
    // ★ 统计总 pin 数，初始化实时计数器（每个 pin 刷新一次；如果太频繁，可把 step_gate 设为更大，比如 64）
    size_t totalPins = 0;
    for (Net* net : db->Nets) if (net) totalPins += net->netPins.size();
    LiveCounter lc;
    lc.init(totalPins, /*step_gate=*/1, /*ms_gate=*/0);  // 逐 pin 刷新；想稍微降频就把 step_gate 调大或 ms_gate>0
    ...
    for (Net* net : db->Nets) {
        ...
        for (int i = 0; i < (int)pins.size(); i++) {
            Pin* p = pins[i];
            Module* m = p ? p->module : nullptr;

            // —— 每处理 1 个 pin（外层 i）就更新一次进度 —— //
            lc.tick(1);
    ...
    std::cout << "\nInitial placement completed." << std::endl;
}
