...
void MyPlacer::initialPlacement(int maxOuter /*=200*/, double tol /*=1e-6*/)
{
    ...
    int N = movable.size();
    if (N == 0) {
        std::cout << "[Init] No movable modules.\n";
        return;
    }

    // ============================================================
    // 2) Initial placement (uniform scatter)
    // ============================================================
    {
        ...
        std::mt19937 rng(7);
        std::uniform_real_distribution<double> rx(llx, urx);
        std::uniform_real_distribution<double> ry(lly, ury);
        ...
        std::cout << "[Init] Uniform random initialization done.\n";
    }

    // ============================================================
    // 3) Allocate solver buffers
    // ============================================================
    Vec X(N), Y(N);
    ...
    for (int iter = 1; iter <= maxOuter; iter++)
    {
        ...
        // -----------------------------
        // D) Assemble total matrix:
        //     C_total = C + C_move + C_hold
        // -----------------------------
        ...
        // H) Visualization
        // -----------------------------
        PLOTTING::plotPlacement("iteration/init_iter_" + std::to_string(iter), db);

        std::cout << "[Init] Iter=" << iter
                  << "   mu=" << mu
                  << "   res=" << solver.error() << "\n";
    }

    std::cout << "\n[Init] Placement Completed.\n";
}
