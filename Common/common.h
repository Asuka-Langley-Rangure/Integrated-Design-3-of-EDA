...
float calc_lipschitz_constant(const std::vector<T> &xt, const std::vector<T> &xt_1, const std::vector<T> &dxt, const std::vector<T> &dxt_1)
{
    ...
    // 防止分母为 0 或极小导致 NaN/Inf
    const float denom = sqrt(squared_sum_denominator);
    if (denom <= 1e-12f || !std::isfinite(denom))
        return 10.0f;  // 返回合理的L，对应步长 0.1（避免过小）

    float lipschitz = sqrt(squared_sum_numerator) / denom;
    if (!std::isfinite(lipschitz) || lipschitz <= 1e-12f)
        lipschitz = 10.0f;  // 返回合理的L，对应步长 0.1（避免过小）
    return lipschitz;
}
