...
void MyPlacer::initialPlacement(bool skipIteration)
{
    ...
    // 如果设置了跳过迭代，尝试加载保存的状态
    if (skipIteration) {
        if (loadInitialPlacementState()) {
            std::cout << "[Init] Loaded saved initial placement state. Skipping iteration.\n";
            lastHPWL = db ? db->calcHPWL() : 0.0;
            return;
        } else {
            std::cout << "[Init] Failed to load saved state. Proceeding with normal iteration.\n";
        }
    }
    ...
    for (int iter = 0; iter < maxOuterIters; ++iter) {
        ...
        // 在第30次迭代后保存状态
        if (iter == 29) {
            if (saveInitialPlacementState()) {
                std::cout << "[Init] Saved state after iteration 30.\n";
            } else {
                std::cout << "[Init] Warning: Failed to save state after iteration 30.\n";
            }
        }
...
void MyPlacer::FillerInit()
{
    ...
    // 从 SiteRows 获取 commonRowHeight（如果未设置）
    float fillerHeight = db->commonRowHeight;
    if (fillerHeight <= 0 && !db->SiteRows.empty()) {
        fillerHeight = static_cast<float>(db->SiteRows.front().height);
        db->commonRowHeight = fillerHeight;  // 保存到 db 中
    }
    
    // 安全检查：如果仍然无效，使用默认值
    if (fillerHeight <= 0) {
        std::cerr << "[WARN] commonRowHeight is invalid, using default value 10.0\n";
        fillerHeight = 10.0f;
        db->commonRowHeight = fillerHeight;
    }
    
    float fillerWidth = fillerArea / fillerHeight;
    
    // 安全检查：确保 fillerWidth 有效
    if (!std::isfinite(fillerWidth) || fillerWidth <= 0) {
        std::cerr << "[ERR] Invalid fillerWidth calculated: " << fillerWidth 
                  << " (area=" << fillerArea << ", height=" << fillerHeight << ")\n";
        fillerWidth = 10.0f;  // 使用默认值
    }
...
bool MyPlacer::saveInitialPlacementState(const std::string& filename)
{
    std::ofstream outFile(filename);
    if (!outFile.is_open()) {
        std::cerr << "[Init] Failed to open file for writing: " << filename << std::endl;
        return false;
    }

    // 保存所有可移动模块的位置
    // 格式：idx name center.x center.y
    int savedCount = 0;
    for (Module* m : db->Nodes) {
        if (!m) continue;
        if (m->isFixed) continue;  // 只保存可移动模块
    ...
    outFile.close();
    std::cout << "[Init] Saved " << savedCount << " module positions to " << filename << std::endl;
    return true;
}

bool MyPlacer::loadInitialPlacementState(const std::string& filename)
{
    std::ifstream inFile(filename);
    if (!inFile.is_open()) {
        std::cerr << "[Init] Failed to open file for reading: " << filename << std::endl;
        return false;
    }

    // 创建索引映射：idx -> Module*
    std::unordered_map<int, Module*> idxToModule;
    for (Module* m : db->Nodes) {
        if (!m) continue;
        idxToModule[m->idx] = m;
    }
    ...
    while (std::getline(inFile, line)) {
        ...
        if (!(iss >> idx >> name >> x >> y)) {
            std::cerr << "[Init] Warning: Failed to parse line: " << line << std::endl;
            continue;
        }
        
        auto it = idxToModule.find(idx);
        if (it != idxToModule.end() && it->second) {
            Module* m = it->second;
            if (!m->isFixed) {  // 只加载可移动模块
                m->center.x = x;
                m->center.y = y;
                loadedCount++;
            }
    ...
    inFile.close();
    std::cout << "[Init] Loaded " << loadedCount << " module positions from " << filename << std::endl;
