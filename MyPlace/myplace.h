...
public:
    ...
    float darkDensity;
    float availableArea;  // 可用面积（扣除宏块和固定单元后的面积）
    VECTOR_2D E;
    float phi;   
};
...
public:
    ...
    void initialPlacement(bool skipIteration = false);
    bool saveInitialPlacementState(const std::string& filename = "initial_placement_state.txt");
    bool loadInitialPlacementState(const std::string& filename = "initial_placement_state.txt");
