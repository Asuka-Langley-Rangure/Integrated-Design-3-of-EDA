...
// 工具函数：把当前解析结果完整导出到 txt
void dumpDefDesignToFile(const DefDesign& design, const std::string& outFile)
{
    std::ofstream ofs(outFile);
    if (!ofs) {
        std::cerr << "[ERR] dumpDefDesignToFile: cannot open file " << outFile << "\n";
        return;
    }
    ...
    // 基本信息
    ofs << "Design Name : " << design.name << "\n";
    ofs << "Version     : " << design.version.version << "\n";
    ofs << "Units       : " << design.units.dbu_per_micron << " DBU/um\n";
    ofs << "DieArea     : (" << design.die_area.x1 << ", " << design.die_area.y1
        << ") - (" << design.die_area.x2 << ", " << design.die_area.y2 << ")\n";
    ofs << "DividerChar : " << design.misc.dividerChar << "\n";
    ofs << "BusBitChars : " << design.misc.busBitChars << "\n\n";

    // Components
    ofs << "=== Components (" << design.components.size() << ") ===\n";
    for (size_t i = 0; i < design.components.size(); ++i) {
        const auto& c = design.components[i];
        ofs << "  [" << i << "] "
            << c.name << "  type=" << c.type
            << "  macro=" << c.macro
            << "  pos=(" << c.x << ", " << c.y << ")"
            << "  orient=" << c.orient
            << "  placed=" << (c.placed ? "true" : "false")
    ...
    for (size_t i = 0; i < design.nets.size(); ++i) {
        ...
        for (size_t j = 0; j < net.connections.size(); ++j) {
            const auto& conn = net.connections[j];
            ofs << "       (" << conn.instance << " " << conn.pin << ")\n";
        }
    }
    ofs << "\n";

    // Rows
    ofs << "=== Rows (" << design.rows.size() << ") ===\n";
    for (size_t i = 0; i < design.rows.size(); ++i) {
        const auto& r = design.rows[i];
        ofs << "  [" << i << "] " << r.name
            << "  site=" << r.site
            << "  origin=(" << r.x << ", " << r.y << ")"
            << "  orient=" << r.orient
            << "  DO " << r.do_count_x << " BY " << r.do_count_y
            << "  STEP (" << r.step_x << ", " << r.step_y << ")\n";
    }
    ofs << "\n";

    // Tracks
    ofs << "=== Tracks (" << design.tracks.size() << ") ===\n";
    for (size_t i = 0; i < design.tracks.size(); ++i) {
        const auto& t = design.tracks[i];
        ofs << "  [" << i << "] dir=" << t.direction
            << "  start=" << t.start
            << "  count=" << t.count
            << "  step=" << t.step
            << "  layer=" << t.layer << "\n";
    ...
    // GCellGrids
    ofs << "=== GCellGrids (" << design.gcells.size() << ") ===\n";
    for (size_t i = 0; i < design.gcells.size(); ++i) {
        const auto& g = design.gcells[i];
        ofs << "  [" << i << "] dir=" << g.direction
            << "  start=" << g.start
            << "  count=" << g.count
            << "  step=" << g.step << "\n";
    }
    ...
    // SpecialNets
    ofs << "=== SpecialNets (" << design.specialnets.size() << ") ===\n";
    for (size_t i = 0; i < design.specialnets.size(); ++i) {
        const auto& sn = design.specialnets[i];
        ofs << "  [" << i << "] SNet " << sn.name
            << "  connections=" << sn.connections.size()
            << "  wires=" << sn.wires.size() << "\n";

        // connections
        for (size_t j = 0; j < sn.connections.size(); ++j) {
            const auto& conn = sn.connections[j];
            ofs << "       CONN (" << conn.instance << " " << conn.pin << ")\n";
        }
        // wires
        for (size_t j = 0; j < sn.wires.size(); ++j) {
            const auto& w = sn.wires[j];
            ofs << "       WIRE #" << j
                << " type=" << w.type
                << " layer=" << w.layer
                << " width=" << w.width
                << " shape=" << w.shape
                << "  points=" << w.points.size() << "\n";
            for (size_t k = 0; k < w.points.size(); ++k) {
                ofs << "            (" << w.points[k].first
                    << ", " << w.points[k].second << ")\n";
            }
        }
    }
    ...
    // Pins
    ofs << "=== Pins (" << design.pins.size() << ") ===\n";
    for (size_t i = 0; i < design.pins.size(); ++i) {
        const auto& p = design.pins[i];
        ofs << "  [" << i << "] " << p.name
            << "  net=" << p.net
            << "  pos=(" << p.x << ", " << p.y << ")"
            << "  layer=" << p.layer
    ...
    std::cerr << "[INFO] DEF design dumped to " << outFile << "\n";
}

void DefParser::dumpToFile(const std::string& outFile) const
{
    dumpDefDesignToFile(design, outFile);
}
